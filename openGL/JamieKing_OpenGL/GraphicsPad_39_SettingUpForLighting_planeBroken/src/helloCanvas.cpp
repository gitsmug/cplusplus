...
void HelloCanvas::SendDataToOpenGL()
{
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, VERTEX_BYTE_SIZE, reinterpret_cast<void*>(arrowByteOffset + sizeof(GLfloat) * 3));
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, theBufferID);
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, VERTEX_BYTE_SIZE, reinterpret_cast<void*>(planeByteOffset + sizeof(GLfloat) * 3));
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, theBufferID);
...
void HelloCanvas::PaintGL()
{
	...
	// Plane
	glBindVertexArray(planeVertexArrayObjectID);
	mat4 planeModelToWorldMatrix = glm::translate(vec3(1.0f, 0.0f, 3.0f));
